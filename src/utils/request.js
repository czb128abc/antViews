...
import { removeEmptyFields } from './processData';

// const hostApi = 'http://47.106.141.90:8001';
const hostApi = '';
...
async function baseRequest(url, config, otherConfig = {}) {
  const { failMsg = '', isUseDvaCommonError = true } = otherConfig;

  const result = await request(url, config);
  result.success = result.code === 200;
  if (!result.success && isUseDvaCommonError) {
    if (codeMessage[result.code]) {
      throw new Error(`${codeMessage[result.code]} ~ ${result.message}`);
    }
...
export function get(url, params, otherConfig = {}, method = 'GET') {
  let newParams = params;
  const { isRemoveEmptyFields = false } = otherConfig;
  if (isRemoveEmptyFields) {
    newParams = removeEmptyFields(params);
